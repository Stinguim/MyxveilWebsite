...
import {
  CATEGORIA_GUIA_LABEL,
  ORDEM_CATEGORIAS_GUIA,
  type GuidePage,
...
export function GuiaSidebar({
  ...
  paginas: Pick<GuidePage, "slug" | "titulo" | "categoria">[];
  slugAtual?: string;
}) {
  const porCategoria = ORDEM_CATEGORIAS_GUIA.map((categoria) => ({
    categoria,
      ...
      {porCategoria.map(({ categoria, paginas: paginasDaCategoria }) => (
        <div key={categoria}>
          <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-neutral-500">
            {CATEGORIA_GUIA_LABEL[categoria]}
          </h3>
